...
class MapGen():
    def __init__(self, ori_xml, baidu_xml, out_file, none_ListFile=None):
        try:
            self.ori = minidom.parse(ori_xml)
            self.baidu = minidom.parse(baidu_xml)
            self.out = codecs.open(out_file, "w")
            if none_ListFile is not None:
                self.noneList = codecs.open(none_ListFile, "w")
            print "generate the map from %s to %s" % (baidu_xml, ori_xml)
        except:
            traceback.print_exc()
            sys.exit(1)
    ...
    def genUpdateMap(self):
        ...
        baiduRoot = self.baidu.documentElement

        for item in baiduRoot.childNodes:
            if item.nodeType == minidom.Node.ELEMENT_NODE:
                itemType = item.getAttribute("type")
                itemName = item.getAttribute("name")
                itemId = item.getAttribute("id")
...
def main():
    if len(sys.argv) < 5:
        print "USAGE: GenMap.py -add/-map public_ori.xml public_baidu.xml out1.txt [out2.txt]"
        sys.exit(1)
